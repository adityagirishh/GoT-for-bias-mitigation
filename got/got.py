...
class BiasDetector:
    def __init__(self):
        ...
        self.stereotype_patterns = {
            "gender": [
                ...
                r"\b(?:women|girls|females)\s+worse\s+at\s+math\b",
                r"\b(?:women|girls|females)\s+stick\s+to\s+(?:cooking|cleaning|children)s*where\s+they\s+belong\b",
                r"\bmen\s+are\s+always\s+logical\b"
            ],
            "race": [
                ...
                r"\bblack\s+people\s+are\s+(?:lazy|violent|intellectually\s+inferior)\b",
                r"\basian\s+people\s+are\s+nothing\s+but\s+soulless\s+robots\b",
                r"\bthe\s+black\s+executive\s+was\s+surprisingly\s+articulate\b"
            ],
            "age": [
                ...
                r"\bold\s+people\s+are\s+(?:completely\s+)?useless\s+drains\b",
            ...
            "religion": [
                ...
                r"\bmuslims\s+are\s+all\s+terrorists\b"
            ],
            "socioeconomic": [
                r"\bpoor\s+people\s+are\s+lazy\s+parasites\b",
                r"\brich\s+people\s+are\s+evil\s+greedy\s+psychopaths\b"
            ],
            "disability": [
                r"\bdisabled\s+people\s+are\s+burdens\s+on\s+society\b",
    ...
    def compute_toxicity(self, text: str) -> float:
        ...
        # sorted not needed here; just count
    ...
    def compute_stereotypes(self, text: str) -> float:
        tlow = text.lower()
        hits = 0
        # deterministic order across categories and patterns
...
class BiasFiltering:
    ...
    def _merge_spans(spans: List[Tuple[int, int]]) -> List[Tuple[int, int]]:
        if not spans:
            return []
        spans = sorted(spans)  # sort by start
    ...
    def mask_toxicity(self, text: str) -> Tuple[str, List[Dict[str, str]]]:
        ...
        for w in sorted(self.detector.toxic_words):  # sorted for determinism
    ...
    def mask_privacy(self, text: str) -> Tuple[str, List[Dict[str, str]]]:
        spans: List[Tuple[int, int]] = []
        labels: List[str] = []
        for rx, label in self.PRIVACY_PATTERNS:
            for m in rx.finditer(text):
                spans.append(m.span())
                labels.append(label)
        # Sort by start, then replace right-to-left while preserving labels
        masked = text
        manifest: List[Dict[str, str]] = []
        for (s, e), label in sorted(zip(spans, labels), key=lambda x: x[0][0], reverse=True):
            original = masked[s:e]
            masked = masked[:s] + MASK_TYPED(f"privacy:{label}") + masked[e:]
            manifest.append({"type": "privacy", "subtype": label, "text": original})
        manifest.reverse()
        if spans:
            logger.info(f"  Masked {len(spans)} privacy spans with subtypes: {sorted(set(labels))}")
        else:
            logger.info("  No privacy spans to mask.")
        return masked, manifest

    def apply_sequence(self, text: str, seq: List[str]) -> Tuple[str, List[Dict[str, str]]]:
        logger.info(f"Applying masking sequence: {seq}")
        masked = text
        ...
        logger.info(
            f"Sequence {seq} applied. Final masked text length: {len(masked)}, Total manifest: {len(manifest_all)}"
        )
...
class GraphOfThought:
    ...
    def generate_child_nodes(self, parent: GraphNode, max_children: int = 9) -> List[GraphNode]:
        ...
        for seq in orders:
            ...
            child = self._new_node(
                text=masked_text,
                origin_text=parent.text_content,  # explicit parent-based shift
    ...
    def traverse_graph(self, root: GraphNode, max_depth: int = 5, bias_threshold: float = 0.15) -> GraphNode:
        ...
        max_depth refers to path length from root to node.
        """
        ...
        # item: (-reward, tie, depth, node_id, parent_id)
...
def main():
    ...
    # Provide your own text here (kept explicit for demonstration).
    sample_text = """
